...
public class EmailAddressDAOTest {
        ...
        private final static Logger LOG = LoggerFactory.getLogger(EmailAddressDAOTest.class);
    ...
    public void init() {
        ...
        otherAddressBean.setAddressName("other.1642705@gmail.com");
            ...
            @Test
    public void testaddEmailAddress() throws SQLException {
        GmailAddressDAO gmailAddressDAO = new GmailAddressDAO();
        int id = gmailAddressDAO.addEmailAddress(senderAddressBean);
        MailAddressBean senderAddressBeanCopy = gmailAddressDAO.getMailAddressBeanByID(id);

        LOG.info("testaddEmailAddress id :" + id );
        assertTrue( senderAddressBeanCopy.equals(senderAddressBean));
    }
    
    
        @Test
    public void testgetAllEmailAddresses() throws SQLException {
        GmailAddressDAO gmailAddressDAO = new GmailAddressDAO();
        gmailAddressDAO.addEmailAddress(senderAddressBean);
        gmailAddressDAO.addEmailAddress(receiverAddressBean);
        gmailAddressDAO.addEmailAddress(otherAddressBean);

        MailAddressBean[] allAddresses = gmailAddressDAO.getAllEmailAddresses();
        LOG.info("testgetAllEmailAddresses:" );
        assertEquals("test getAllEmailAddresses FAIL", 3, allAddresses.length);
    }
